...
var visitor = types.PathVisitor.fromMethodsObject({
  ...
  visitExportDeclaration: function (path) {
    if(path.value.default && path.value.declaration.type === 'ClassDeclaration') {
      this.traverse(path);
      var classDeclaration = path.value.declaration;
      var exportClause = path.value;
      assert.ok(classDeclaration.declarations.length === 1);
      exportClause.declaration = classDeclaration.declarations[0].id;
      path.replace(classDeclaration);
      path.insertAfter(exportClause);
    } else {
      this.traverse(path);
    }
  }
